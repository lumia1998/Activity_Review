...
test('概览域名语义分类修改应命中后端 domain-semantic-rule 接口，而不是前端本地假实现', async () => {
  const [overviewSource, clientSource, backendSource] = await Promise.all([
  ...
  assert.match(overviewSource, /invoke\('set_domain_semantic_rule'/);
  ...
  assert.match(backendSource, /@router\.post\('\/domain-semantic-rule'\)/);
  assert.match(backendSource, /return set_domain_semantic_rule\(payload\.domain, payload\.semanticCategory, payload\.syncHistory\)/);
});
...
test('概览统计命令应注册为 get_overview_stats', async () => {
  ...
  assert.match(clientSource, /case 'get_overview_stats':[\s\S]*request\(`\/api\/stats\/overview/);
  assert.match(appSource, /cache\.invalidate\('overview'\)/);
});
