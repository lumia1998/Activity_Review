...
test('侧边栏录制控制应命中真实 runtime API，并广播录制状态事件', async () => {
  const [sidebarSource, clientSource] = await Promise.all([
  ...
  assert.match(sidebarSource, /await invoke\('resume_recording'\)/);
  assert.match(sidebarSource, /await invoke\('pause_recording'\)/);
  assert.match(clientSource, /emitLocalEvent\('recording-state-changed'/);
  assert.match(clientSource, /case 'get_recording_state':[\s\S]*request\('\/api\/runtime\/recording-state'\)/);
  assert.match(clientSource, /case 'pause_recording':[\s\S]*request\('\/api\/runtime\/pause-recording'/);
  assert.match(clientSource, /case 'resume_recording':[\s\S]*request\('\/api\/runtime\/resume-recording'/);
});
