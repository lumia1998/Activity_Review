...
Activity Review 會在後台持續記錄你當天使用過的應用、訪問過的網站、關鍵窗口和屏幕內容，再把這些離散片段整理成一條**可回看、可追問、可復盤**的工作軌跡。
...
Activity Review 更像一套面向個人的工作留痕系統：
...
<img src="docs/Introduction_tw/概览.png" alt="Activity Review 今日概覽" />

概覽頁會把當天的總時長、辦公時長、瀏覽器使用、網站訪問、按小時活躍度和應用分佈放在同一屏裡，適合先快速判斷今天的工作重心。

### 工作助手

<img src="docs/Introduction_tw/助手.png" alt="Activity Review 工作助手" />
...
Activity Review 的核心始終是**本地記錄**，AI 的作用是把這些記錄變得更容易閱讀、搜索和復盤。
...
從 [Releases](https://github.com/lumia1998/Acticity_Review/releases/latest) 下載最新版。
...
然後到 `系統設置 > 隱私與安全性 > 屏幕錄製` 為 Activity Review 開啟權限。

如果你要使用 `桌面化身 Beta`，還需要同時為 Activity Review 開啟 `輔助功能` 權限，以便讀取前台窗口狀態並及時切換桌寵表情。
</details>
...
backend/app/          FastAPI 後端（統計、時間線、配置、運行時、助手）
desktop/              Python 桌面宿主（PyWebView 啟動與窗口橋接）
```
